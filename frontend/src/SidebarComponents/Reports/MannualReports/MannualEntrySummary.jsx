...
const MannualEntrySummary = () => {
  ...
  const getTimeDiff = (inTime, outTime) => {
    ...
    try {
      ...
      if (diff < 0) diff += 24 * 3600; // Handle overnight shift
  ...
  const filteredReport = manualReport.filter(
    (x) =>
  ...
  const handleCopy = () => {
    const header = ["Sl.No", "Employee", "Company", "Punch Date", "Day", "Check in", "Check out", "Total Hours"].join("\t");
  ...
  const handleExcel = () => {
    const data = filteredReport.map((item, i) => {
      ...
      return {
        ...
        "Punch Date": dt ? dt.toLocaleDateString() : "—",
        Day: dt ? dt.toLocaleDateString("en-US", { weekday: "long" }) : "—",
    ...
    XLSX.utils.book_append_sheet(wb, ws, "ManualEntrySummary");
    XLSX.writeFile(wb, "ManualEntrySummary.xlsx");
  };

  const handlePDF = () => {
    const doc = new jsPDF("landscape");
    doc.text("Mannual Entry Summary", 14, 14);
    autoTable(doc, {
      startY: 20,
      head: [["Sl.No", "Employee", "Punch Date", "Day", "Check in", "Check out", "Total Hours"]],
    ...
    doc.save("ManualEntrySummary.pdf");
  };
              ...
              <table className="w-full text-lg border-collapse">
                <thead className="bg-[oklch(0.94_0.001_106.424)] text-[oklch(0.44_0.001_106.424)]">
                  <tr>
                    ...
                    <th className="p-2 font-semibold whitespace-nowrap hidden md:table-cell">Punch Date</th>
                    <th className="p-2 font-semibold whitespace-nowrap hidden md:table-cell">Punch Day</th>
