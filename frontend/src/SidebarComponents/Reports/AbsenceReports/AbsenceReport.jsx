...
const AbsenceReport = () => {
  ...
  const handleGenerateReport = async () => {
    try {
      ...
      // Absence filter: only show those with NO in_time AND NO out_time
      const filteredResult = res.data.filter(item => !item.in_time && !item.out_time);
      setAbsenceReport(filteredResult);
      setCurrentPage(1);
      setOpenModal(true);
    } catch (err) {
  ...
  const filteredReport = absenceReport.filter(
    (x) =>
  ...
  const selectedItem = absenceReport.find((item) => item.id === selectedId);

  const handleCopy = () => {
    const header = ["Sl.No", "Employee", "Employee ID", "Company", "Date", "Status"].join("\t");
  ...
  const handleExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "AbsenceReport");
    XLSX.writeFile(wb, "AbsenceReport.xlsx");
  };

  const handlePDF = () => {
    const doc = new jsPDF("landscape");
    doc.text("Absence Report", 14, 14);
    autoTable(doc, {
      startY: 20,
      head: [["Sl.No", "Employee", "Employee ID", "Company", "Date", "Status"]],
          ...
          <div className="mt-6 bg-white shadow-xl rounded-xl border border-[oklch(0.8_0.001_106.424)] p-6 ">
              ...
              <table className="w-full text-lg border-collapse">
                <thead className="bg-[oklch(0.94_0.001_106.424)] text-[oklch(0.44_0.001_106.424)]">
                  <tr>
                    ...
                    <th className="p-2 font-semibold hidden md:table-cell ">SL.NO</th>
                    <th className="p-2 font-semibold hidden md:table-cell ">Employee ID</th>
                    <th className="p-2 font-semibold whitespace-nowrap hidden xl:table-cell">Date</th>
                    <th className="p-2 font-semibold whitespace-nowrap hidden xl:table-cell">Day</th>
                    <th className="p-2 font-semibold hidden sm:table-cell">Status</th>
                ...
                <tbody>
                  {currentData.length === 0 ? (
                    ...
                    currentData.map((item, index) => {
                      const dt = item.created_at ? new Date(item.created_at) : null;
                      const isFirst = index === 0 || currentData[index - 1].employee_name !== item.employee_name;
                      return (
                        <tr
                          key={item.id}
                        ...
                        >
                          {isFirst ? (
                             <td
                              rowSpan={getRowSpan(currentData, index, "employee_name")}
                              className="p-2 whitespace-nowrap border-r border-[oklch(0.8_0.001_106.424)]"
                             >
                               {item.employee_name}
                             </td>
                          ) : null}
                          <td className="p-2 hidden md:table-cell">{startIndex + index + 1}</td>
                          <td className="p-2 hidden md:table-cell">{item.employee_code || "—"}</td>
                          <td className="p-2 whitespace-nowrap hidden xl:table-cell">{dt ? dt.toLocaleDateString() : "—"}</td>
                          <td className="p-2 hidden xl:table-cell">{dt ? dt.toLocaleDateString("en-US", { weekday: "long" }) : "—"}</td>
                          <td className="p-2 whitespace-nowrap hidden sm:table-cell">
                            <span className="bg-red-100 text-red-700 px-2 py-1 rounded-full text-sm">Absent</span>
                          </td>
                          <td className="p-2 ">
            ...
            <div className="flex justify-center md:justify-between items-center mt-4 text-sm flex-wrap gap-6">
              <span>
                ...
                {Math.min(startIndex + entriesPerPage, filteredReport.length)} of {filteredReport.length} entries
              </span>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 overflow-y-auto">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto p-6" style={{ scrollbarWidth: "none" }}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-lg">
                ...
                <div><p className={labelStyle}>Company</p><p className={inputStyle}>{selectedItem.company_name || "—"}</p></div>
                ...
                <div><p className={labelStyle}>Status</p><p className={`${inputStyle} text-red-600`}>Absent</p></div>
              </div>
            </div>
          </div>
        )}
