...
const InOutReport = () => {
  ...
  const filteredReport = inOutReport.filter(
    (x) =>
  ...
  const handleCopy = () => {
    const header = ["Sl.No", "Employee", "Company", "Date", "Day", "Check in", "Location"].join("\t");
  ...
  const handleExcel = () => {
    ...
    XLSX.writeFile(wb, "InOutReport.xlsx");
  };

  const handlePDF = () => {
    const doc = new jsPDF("landscape");
    doc.text("In Out Report", 14, 14);
    autoTable(doc, {
      startY: 20,
      head: [["Sl.No", "Employee", "Company", "Date", "Day", "Check in", "Location"]],
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-lg">
                ...
                <div>
                  ...
                  <p className={inputStyle}>{selectedItem.status || "Pending"}</p>
                </div>
              </div>
            </div>
          </div>
