...
const AllTransactionReport = () => {
  ...
  const handleGenerateReport = async () => {
    try {
      ...
      setAllTransactionReport(res.data);
      setCurrentPage(1);
      setOpenModal(true);
    } catch (err) {
  ...
  const filteredReport = allTransactionReport.filter(
    (x) =>
  ...
  const selectedItem = allTransactionReport.find((item) => item.id === selectedId);

  const handleCopy = () => {
    const header = ["Sl.No", "Employee", "Company", "Date", "Check in", "Check out", "Total Hours", "Status"].join("\t");
  ...
  const handleExcel = () => {
    ...
    XLSX.writeFile(wb, "AllTransactionReport.xlsx");
  };

  const handlePDF = () => {
    const doc = new jsPDF("landscape");
    doc.text("All Transaction Report", 14, 14);
    autoTable(doc, {
      startY: 20,
      head: [["Sl.No", "Employee", "Company", "Date", "Check in", "Check out", "Total Hours", "Status"]],
    ...
    doc.save("AllTransactionReport.pdf");
  };
