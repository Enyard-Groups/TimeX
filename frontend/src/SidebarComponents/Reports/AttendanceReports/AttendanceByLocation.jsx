...
const AttendanceByLocation = () => {
  ...
  const [attendanceByLocation, setAttendanceByLocation] = useState([]);
  ...
  useEffect(() => {
    ...
    setAttendanceByLocation(stored);
  }, []);
  ...
  const filteredattendanceByLocation = filteredReport.filter(
    (x) =>
  ...
  const currentattendanceByLocation = filteredattendanceByLocation.slice(
    startIndex,
    endIndex,
  );
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredattendanceByLocation.length / entriesPerPage),
  );

  const selectedItem = attendanceByLocation.find(
          ...
          <div className="bg-gradient-to-br from-white to-slate-50 rounded-2xl overflow-hidden border border-blue-100/50 shadow-xl animate-in fade-in duration-500">
            ...
            >
              <table className="w-full text-[17px]">
                ...
                <tbody>
                  {currentattendanceByLocation.length === 0 ? (
                    <tr>
                      <td
                        colSpan="11"
                    ...
                    currentattendanceByLocation.map((item, index) => (
                      <tr
                        key={item.id}
            ...
            <div className="p-6 border-t border-blue-100/30 flex flex-col sm:flex-row justify-between items-center gap-6">
              <span className="text-sm xl:text-base text-gray-600">
                ...
                <span className="text-gray-900 font-semibold">
                  {filteredattendanceByLocation.length === 0
                    ? "0"
                    : startIndex + 1}
                </span>{" "}
                to{" "}
                <span className="font-bold text-gray-900">
                  {Math.min(endIndex, filteredattendanceByLocation.length)}
                </span>{" "}
                of{" "}
                <span className="font-bold text-gray-900">
                  {filteredattendanceByLocation.length}
                </span>{" "}
                entries
              </span>
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <p
                    className={`py-1 px-3 w-fit rounded xl:text-lg  font-semibold border ${selectedItem.status === "Approved" ? "bg-green-100 text-green-700 border-green-200" : selectedItem.status === "Rejected" ? "bg-red-100 text-red-700 border-red-200" : "bg-yellow-100 text-yellow-700 border-yellow-200"}`}
                  >
                    {selectedItem.status}
                  </p>
                </div>
