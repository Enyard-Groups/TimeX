    ...
    <div className="mb-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between mb-6 gap-4 pl-10 lg:pl-0">
        <h1 className="flex items-center gap-2 h-[30px] text-lg xl:text-xl font-semibold text-gray-800">
          ...
          <FaAngleRight className="text-blue-500 text-base" />
          <div className="text-blue-600">Claim Approval</div>
        </h1>
      </div>
              ...
              {loading ? (
                ...
                currentData.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2 text-center hidden sm:table-cell text-gray-600">
                      {item.claim_category}
                    </td>
                    <td className="px-4 py-2 text-center hidden lg:table-cell text-gray-600 text-sm">
                      {item.date
                        ? new Date(item.date).toLocaleDateString()
                        : "-"}
                    </td>
                    <td className="px-4 py-2 text-center hidden xl:table-cell text-gray-600 max-w-xs truncate italic">
                      {item.purpose || "NIL"}
                    </td>
          ...
          <div className="bg-gradient-to-br from-white to-slate-50 rounded-2xl shadow-2xl border border-blue-100/50 w-full max-w-5xl max-h-[90vh] overflow-y-auto p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                ...
                { label: "Claim Category", value: selectedItem.claim_category },
                {
                  label: "Claim Date",
                ...
                <div key={idx}>
                  <label className="text-xs xl:text-base font-bold uppercase tracking-wider text-slate-500 mb-2 block">
                    {field.label}
                  </label>
                  {field.isStatus ? (
                    <span className="px-4 py-1.5 bg-amber-100 text-amber-700 rounded-lg font-bold text-sm xl:text-base shadow-sm inline-block">
                      {field.value}
                    </span>
                  ) : (
              ...
              <div className="md:col-span-2">
                <label className="text-xs xl:text-base font-bold uppercase tracking-wider text-rose-600 mb-2 block">
                  Rejection Reason
                </label>
                <textarea
