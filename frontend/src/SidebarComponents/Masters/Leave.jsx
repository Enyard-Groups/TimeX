...
const Leave = () => {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${API_BASE}/master/leave-types/${id}`, {
        headers: getHeaders(),
      });
      ...
      toast.error(
        error.response?.data?.message || "Unable to delete leave type",
      );
    }
  };
  ...
  const handleExcel = () => {
    const excelData = filteredleave.map((item, index) => ({
      "SL.NO": index + 1,
      Name: item.name,
    ...
    XLSX.writeFile(workbook, "LeaveData.xlsx");
  };
  ...
  const handlePDF = () => {
    ...
    filteredleave.forEach((item, index) => {
      const row = [
        index + 1,
        item.name,
        item.code,
            ...
            <table className="w-full text-[17px]">
              <thead>
                <tr className="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-blue-100/50">
                  ...
                  <th className="px-6 py-3 text-center text-gray-700 font-semibold hidden md:table-cell">
                    LeaveType Code
                  </th>
                  ...
                  <th className="px-6 py-3 text-center text-gray-700 font-semibold hidden lg:table-cell">
                    Status
                  </th>
                  <th className="px-6 py-3 text-center text-gray-700 font-semibold">
                    Action
                    ...
                    >
                      ...
                      <td className="px-4 py-3 hidden lg:table-cell text-center">
                        <div className="flex justify-center">
                          <span
                            className={`px-3 py-1 rounded-full text-sm lg:text-base 3xl:text-2xl font-semibold border ${item.isActive ? "bg-green-100 text-green-700 border-green-300" : "bg-gray-100 text-gray-700 border-gray-300"}`}
                          >
