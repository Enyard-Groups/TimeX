...
const ClaimRequest = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (editId) {
                const res = await axios.put(`${API_BASE}/requests/claim/${editId}`, formData);
                setClaims((prev) =>
    ...
    const filteredClaims = claims.filter((x) =>
    ...
    const totalPages = Math.max(1, Math.ceil(filteredClaims.length / entriesPerPage));

    const handleCopy = () => {
        const header = ["Employee", "Category", "Date", "Amount", "Purpose", "Created Date"].join("\t");
    ...
    const handlePDF = () => {
        ...
        const tableColumn = ["Employee", "Category", "Date", "Amount", "Purpose", "Created Date"];
            ...
            <div className="mt-6 bg-white shadow-xl rounded-xl border border-[oklch(0.8_0.001_106.424)] p-6">
                    ...
                    <table className="w-full text-lg border-collapse">
                        <thead className="bg-[oklch(0.94_0.001_106.424)] text-[oklch(0.44_0.001_106.424)]">
                            <tr>
                                ...
                                <th className="py-2 px-6 font-semibold hidden sm:table-cell">Category</th>
                                <th className="py-2 px-6 font-semibold hidden lg:table-cell">Date</th>
                                <th className="py-2 px-6 font-semibold hidden xl:table-cell">Amount</th>
                            ...
                            {currentClaims.length === 0 ? (
                                ...
                                currentClaims.map((item) => (
                                    ...
                                    >
                                        ...
                                        <td className="py-2 px-6 hidden sm:table-cell">{item.claim_category}</td>
                                        <td className="py-2 px-6 hidden lg:table-cell">{item.date}</td>
                                        <td className="py-2 px-6 hidden xl:table-cell">{item.amount}</td>
                ...
                <div className="flex justify-center md:justify-between items-center mt-4 text-sm flex-wrap gap-6">
                    <span>
                        ...
                        {Math.min(endIndex, filteredClaims.length)} of {filteredClaims.length} entries
                    </span>
                ...
                >
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-6xl p-6 text-[oklch(0.147_0.004_49.25)] overflow-y-auto max-h-[90vh]">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                <SearchDropdown
                                    label={<>Claim Category <span className="text-red-500">*</span></>}
                                    name="claim_category"
                            ...
                            <div>
                                <label className={labelStyle}>Amount <span className="text-red-500">*</span></label>
                                <input
                                    name="amount"
                                    type="number"
